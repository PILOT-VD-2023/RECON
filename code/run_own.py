...
sys.path.append('/data/xcwen/Commit-Beyond-Vulnerability/EPVD_PrototypeV2_patience_09')
import numpy as np
import torch
...
def train(args, train_dataset, train_vul_dataset, model, tokenizer):
    """ Train the model """ 
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)
    train_vul_sampler = RandomSampler(train_vul_dataset) if args.local_rank == -1 else DistributedSampler(train_vul_dataset)
    # train_vul_sampler = RandomSampler(train_vul_dataset) if args.local_rank == -1 else DistributedSampler(train_vul_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, 
                                  batch_size=args.train_batch_size,num_workers=4,pin_memory=True)
    
    train_vul_dataloader = DataLoader(train_vul_dataset, sampler=train_vul_sampler, 
                                  batch_size=args.train_batch_size,num_workers=4,pin_memory=True)
    # args.max_steps = args.epoch*len(train_dataloader)
    args.max_steps=args.epoch*(len(train_dataloader)+len(train_vul_dataloader))
    args.save_steps = len(train_dataloader) +len(train_vul_dataloader) 
    args.warmup_steps = len(train_dataloader) +len(train_vul_dataloader)
    args.logging_steps = len(train_dataloader) +len(train_vul_dataloader)
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        # else: 
            # bar = tqdm(train_vul_dataloader,total=len(train_vul_dataloader))
            # args.save_steps=len( train_vul_dataloader)
            # args.logging_steps=len( train_vul_dataloader)
        # bar = tqdm(train_dataloader, total=len(train_dataloader))
        tr_num = 0
        train_loss = 0
        for step, batch in enumerate(bar):
            ...
            bar.set_description("Initial epoch {} loss {}".format(idx, avg_loss))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step_add % args.logging_steps == 0:

                    logging_loss = tr_loss
                    tr_nb = global_step

                # if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step_add % args.save_steps == 0:
                    ...
                    # # evaluate test_vul
                    # if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                    #     results = evaluate_vul(args, model, tokenizer, eval_when_training=True)
                    #     for key, value in results.items():
                    #         logger.info("  %s = %s", key, round(value, 4))
                    # if results['eval_f1'] > best_f1:  
                    #     patience_flag = 1
        ...
        for step, batch in enumerate(bar):
            ...
            if avg_loss == 0:
                avg_loss = tr_loss
            avg_loss = round(train_loss/tr_num, 5)
            bar.set_description("Vul epoch {} loss {}".format(idx, avg_loss))
...
def evaluate_vul(args, model, tokenizer, eval_when_training=False):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_output_dir = args.output_dir

    eval_dataset = TextDataset(tokenizer, args, args.eval_vul_data_file)
...
def test(args, model, tokenizer):
    ...
    with open(os.path.join(args.output_dir, "predictions_not.txt"), 'w') as f:
    # with open(os.path.join(args.output_dir, "predictions_all_not.txt"), 'w') as f:
...
def main():
    ...
    parser.add_argument("--train_vul_data_file", default=None, type=str, required=True,
                        help="The input training data file (a text file).")
    ## Other parameters
    ...
    parser.add_argument("--test_data_file", default=None, type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--eval_vul_data_file", default=None, type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    ...
    if args.do_train:
        ...
        train_vul_dataset = TextDataset(tokenizer, args,args.train_vul_data_file)

        if args.local_rank == 0:
            torch.distributed.barrier()

        train(args, train_dataset, train_vul_dataset, model, tokenizer)
